...
namespace Divie
{
    public delegate void newFrameHandler(object source, newFrameArgs e);
    public class newFrameArgs : EventArgs
    {
        private Bitmap EventFrame;
        public newFrameArgs(Bitmap bitmap)
        {
            EventFrame = bitmap;
        }
        public Bitmap GetBitmap() { return EventFrame; }
    }
    ...
    class CamControll
    {
        ...
        public event newFrameHandler newframeAvailable;
        private AudioControll audio;

        #region Settings
        int videoBitrate = 12*1000*1000;
        ...
        public void VideoCaptureDevice_NewFrame(object sender, Accord.Video.NewFrameEventArgs eventArgs)
        {
            ...
            Task.WaitAll(
                Task.Run(() =>
                {
                    ...
                    newframeAvailable(this, new newFrameArgs(tempResized)); //fyrer av event new frame er tilgjenglig. Abonneres på av Form1
                })
            );
        ...
        private void setFrameSize(object sender, Accord.Video.NewFrameEventArgs eventArgs)
        {
            if (eventArgs.Frame.Height != 0)
            {
                camVars.CamframeHeigth = eventArgs.Frame.Height;
                camVars.CamframeWidth = eventArgs.Frame.Width;
            }
        }
    }
}
